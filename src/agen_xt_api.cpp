...
xt::pyarray<real_t> py_process_trades_json(const char* json_str, const int interval, const timestamp_us_t& start_ts)
{
	return process_trades_json(json_str, interval, start_ts);
}

xt::pyarray<real_t> py_preprocess_single(const char* symbol, const xt::pyarray<real_t>& a_orig, const int timeframe, const int interval, const bool ext_hours)
{
	const bool training = timeframe == interval;

	xt::xarray<real_t> a_step1 = process_step1_single(symbol, a_orig, training, timeframe, interval, ext_hours);

	xt::xarray<real_t> o_results = process_step2_single(symbol, a_orig, training, timeframe, interval, ext_hours);
...
PYBIND11_MODULE(agen_xt_api, m)
{
	...
	m.def("process_trades_json_debug", [](const char* json_str, const int interval, const timestamp_us_t& start_ts) -> auto
	{
	...
	m.def("preprocess_single_debug", [](const char* symbol, const xt::pyarray<real_t>& a_orig, const int timeframe, const int interval, const bool ext_hours) -> auto
	{
		py::scoped_ostream_redirect redir(std::cout, py::module::import("sys").attr("stdout"));
